...
export class ServicePrincipalService<T, ID> implements CrudOperations<T, ID> {
  ...
  public incluir(t: T) {
    return this.httpClient.post(
      `${CLINICA_MEDICA_API}` + this.base + "/incluir",
      t
    );
  }
  ...
  public alterar(t: T) {
    return this.httpClient.put(
      `${CLINICA_MEDICA_API}` + this.base + "/alterar",
      t
    );
  }
  ...
  public excluir(id: ID) {
    return this.httpClient.delete(
      `${CLINICA_MEDICA_API}` + this.base + "/excluir/" + id
    );
  }
  ...
  public pesquisar(p: T) {
    ...
    return this.httpClient.get(
      `${CLINICA_MEDICA_API}` + this.base + "/pesquisar",
      { params }
    );
  }
  ...
  public buscarPeloId(id: ID) {
    return this.httpClient.get(
      `${CLINICA_MEDICA_API}` + this.base + "/buscarPeloId/" + id
    );
  }

  public buscarTodos() {
    return this.httpClient.get(
      `${CLINICA_MEDICA_API}` + this.base + "/buscarTodos"
    );
  }
    ...
    const res = pickBy(
      ...
      function(value: any) {
        ...
        if (isArray(value)) {
          ...
          definido =
            !isUndefined(value) && !isNull(value) && !isEmpty(trim(value));
        }
        return definido;
      }
    );
