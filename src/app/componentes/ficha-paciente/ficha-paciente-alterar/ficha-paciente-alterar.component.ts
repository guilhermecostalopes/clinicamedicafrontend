...
@Component({
  ...
  styleUrls: ['./ficha-paciente-alterar.component.css']
})
export class FichaPacienteAlterarComponent extends PrincipalComponente {
  ...
  public salvarForm() {
    if (this.fichaPacienteAlterarFormGroup.invalid) {
      ...
      } else if (this.fichaPacienteAlterarFormGroup.get('nomePaciente').errors != null) {
        if (this.fichaPacienteAlterarFormGroup.get('nomePaciente').errors.required) {
          super.mensagemTela('ERROR', 'Nome do paciente é obrigatório !');
        } else if (this.fichaPacienteAlterarFormGroup.get('nomePaciente').errors.minlength) {
          super.mensagemTela('ERROR', 'Nome do paciente deve ter no mínimo de 5 caracteres !');
        } else if (this.fichaPacienteAlterarFormGroup.get('nomePaciente').errors.maxlength) {
          super.mensagemTela('ERROR', 'Nome do paciente deve ter no máximo de 255 caracteres !');
        }
      } else if (this.fichaPacienteAlterarFormGroup.get('numeroCarteiraPlano').errors != null) {
        if (this.fichaPacienteAlterarFormGroup.get('numeroCarteiraPlano').errors.required) {
          super.mensagemTela('ERROR', 'Número da carteira do paciente é obrigatório !');
        } else if (this.fichaPacienteAlterarFormGroup.get('numeroCarteiraPlano').errors.minlength) {
